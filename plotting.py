...
def plot_taxavec2(filtered_df, title, c1, c2, xlabel, ylabel, W, H, subplot_ax, skipx=False):
    ...
    for label in range(H.shape[0]):
        u = H[label,[col1_index, col2_index]]
        rgba = _get_color(label, 0, H.shape[0]-1)
        max_W = np.max(W[:, label])
        ax.plot([0, u[0] * max_W], [0, u[1] * max_W], color=rgba, linewidth=2)
        # _draw_subspace_line(ax,u,maxx,maxy,maxz,label,0,H.shape[0]-1)

    ax.scatter(filtered_df[c1], filtered_df[c2], cmap="Set1")
